        ...
        <v-row justify="center" class="mt-5 mx-0">
            <v-card class="white my-5 mx-1" max-width="350" v-for="topStreamer in listofTopStreamers" :key="topStreamer.id">
                <router-link :to="`/streamview/${topStreamer.user_name}?streamer_ID=${topStreamer.user_id}`"><img :src="`${topStreamer.thumbnail_url}`" alt=""></router-link>
                    <div class="px-2 py-1">
                        <h4 class="mb-2 purple--text font-weight-semi-bold">{{topStreamer.user_name}}</h4>
                        <v-row justify="space-between" >
                            <v-col cols="10">
                            <h5 class="pt-1 font-weight-medium">{{topStreamer.stream_title}}</h5> 
                        </v-col>
                        <v-col cols="2" class="rera">
                            <v-icon color="purple" class="mb-1">mdi-account-heart-outline</v-icon>
                            <h5 class="font-weight-regular rara float-right">{{topStreamer.viewer_count}}</h5>
                        </v-col>
                        </v-row>
                         <v-col class="px-0 py-0" justify="start"><h6 class="ml-2 roro">Hot Pool</h6></v-col>
                    </div>
...
export default {
   ...
   methods:{
        fetchToptreamers: function(){
            let fetchLink = 'https://api.twitch.tv/helix/streams?game_id=116747788&first=40';
